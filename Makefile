...
build-web-base-1.5:
	docker build -t thespaghettidetective/web:base-1.5 -f web/Dockerfile.base web

build-images:
	DOCKERFILE_VERSION=.dev docker-compose build --build-arg with_sqlite=1 --build-arg with_node=1 --build-arg home=/home/user --build-arg user=user --build-arg group=user --build-arg uid=$(shell id -u) --build-arg gid=$(shell id -g)

build-web-and-tasks:
	DOCKERFILE_VERSION=.dev docker-compose build --build-arg with_sqlite=1 --build-arg with_node=1 --build-arg home=/home/user --build-arg user=user --build-arg group=user --build-arg uid=$(shell id -u) --build-arg gid=$(shell id -g) web tasks
