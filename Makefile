.PHONY: build-web-base-1.3 build-images build-static frontdev-up vue-live vue-static shell dbshell

BASENAME = $(shell basename $(shell pwd) | tr '[:upper:]' '[:lower:]')

build-web-base-1.3:
	docker build -t thespaghettidetective/web:base-1.3 -f web/Dockerfile.base web
