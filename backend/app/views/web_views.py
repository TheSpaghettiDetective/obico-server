...
def upload_print(request):
    if request.method == 'POST':
        ...
        video_url, _ = save_file_obj(f'uploaded/{video_path}', request.FILES['file'], settings.PICS_CONTAINER, request.user.syndicate.name, long_term_storage=False)
        preprocess_timelapse.delay(request.user.id, video_url, video_path, request.FILES['file'].name)
