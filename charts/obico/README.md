...
| mlApi | object | `{"command":["gunicorn","--bind=0.0.0.0:3333","--workers=1","--timeout=120","--access-logfile=-","wsgi"],"enabled":true,"externalHost":"","gpu":false,"gpuResources":{"nvidia.com/gpu":1},"replicaCount":1,"resources":{"limits":{"cpu":"2","memory":"2Gi"},"requests":{"cpu":"500m","memory":"1Gi"}},"securityContext":{}}` | ML inference API (AI failure detection). Can be disabled. |
| mlApi.command | list | `["gunicorn","--bind=0.0.0.0:3333","--workers=1","--timeout=120","--access-logfile=-","wsgi"]` | Command for the gunicorn inference server |
...
| redis | object | `{"enabled":true,"externalUrl":"","image":{"pullPolicy":"IfNotPresent","repository":"redis","tag":"8.8-alpine"},"resources":{"limits":{"cpu":"200m","memory":"512Mi"},"requests":{"cpu":"50m","memory":"256Mi"}},"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"runAsNonRoot":true,"runAsUser":999}}` | Bundled Redis (Celery broker + Django Channels + cache) |
...
| redis.resources | object | `{"limits":{"cpu":"200m","memory":"512Mi"},"requests":{"cpu":"50m","memory":"256Mi"}}` | Resource requests/limits for the redis container. The bundled Redis sits flat around 250Mi in steady state — right at the old 256Mi limit — so the request is raised to 256Mi to schedule for the real footprint and the limit to 512Mi to leave headroom before the OOM killer. |
...
| tasks | object | `{"command":["celery","-A","config","worker","--beat","--schedule","/data/celerybeat-schedule","-l","info","-c","2","-Q","realtime,celery"],"resources":{"limits":{"cpu":"1","memory":"2Gi"},"requests":{"cpu":"100m","memory":"512Mi"}}}` | Celery worker (runs in the same pod as the web server) |
...
| tasks.resources | object | `{"limits":{"cpu":"1","memory":"2Gi"},"requests":{"cpu":"100m","memory":"512Mi"}}` | Resource requests/limits for the tasks container. The post-print compile_timelapse task shells out to ffmpeg/libx264; at a 512Mi limit the kernel OOM-kills the ffmpeg child on every compile. Because the Celery parent (PID 1) survives, the container never restarts and nothing surfaces in kubectl — prints just silently never get their timelapse. Observed steady state is around 700Mi with pod-lifetime peaks near 745Mi, so the limit is raised to 2Gi (headroom above the peak) and the CPU limit to a full core so the encode is not throttled for minutes. |
| tolerations | list | `[]` |  |
