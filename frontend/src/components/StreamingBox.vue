...
export default {
  ...
  this.$swal.Prompt.fire({
    ...
    html: `
      <p>${this.$i18next.t("Because you are now on the")} <a target="_blank" href="${getDocRoot('/user-guides/upgrade-to-pro/?source=basic_streaming')}">${this.$i18next.t("{brandName} Cloud Free plan",{brandName:this.$syndicateText.brandName})}</a>:</p>
      <ul>
      ...
      <p>${this.$i18next.t("Support the {brandName} project by",{brandName:this.$syndicateText.brandName})} <a href="${getDocRoot('/ent_pub/pricing/?source=basic_streaming')}">${this.$i18next.t("upgrading to the Pro plan for little more than 1 Starbucks a month.")}</a></p> ${this.$i18next.t("The Pro plan offers many perks, including the")} <a target="_blank" href="${getDocRoot('/user-guides/webcam-streaming-for-human-eyes/?source=basic_streaming')}">${this.$i18next.t("Premium Streaming")}</a>:</p>
      <ul>
    ...
    slowLinkClicked() {
      ...
      this.$swal.Prompt.fire({
        ...
        html: `
          ...
          <p>${this.$i18next.t("Check out")} <a target="_blank" href="${getDocRoot('/user-guides/webcam-feed-is-laggy/')}">${this.$i18next.t("the step-by-step trouble-shooting guide.")}</a></p>
        `,
        showCloseButton: true,
      });
    },
    ...
    showMutedStatusDescription(event) {
      ...
      this.$swal.Prompt.fire({
        ...
        html: `
          ...
          <br>
          <p>${this.$i18next.t("Check")} <a target="_blank" href="${getDocRoot('/user-guides/webcam-feed-is-laggy')}">${this.$i18next.t("this step-by-step troubleshooting guide")}</a>.</p>
        `,
        showCloseButton: true,
      })
    },
