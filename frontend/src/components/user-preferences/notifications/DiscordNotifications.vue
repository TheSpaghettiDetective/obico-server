<template>
  <section v-if="notificationChannel.channelInfo">
    ...
    <div class="form-group row mb-4">
      <label for="id_discord_webhook" class="col-12 col-form-label">Webhook URL</label>
  ...
  created() {
    if (!this.notificationChannel.channelInfo) {
      ...
      if (this.notificationChannel.channelInfo.config) {
        this.webhookUrl = this.notificationChannel.channelInfo.config.webhook_url
      }
    }
  },

  watch: {
    webhookUrl(newValue, oldValue) {
      if (oldValue === null) {
        return
      }
