  ...
  <section v-if="notificationChannel.channelInfo">
    ...
    <div v-if="pushoverEnabled">
      ...
      <div class="form-group row mb-4">
        <label for="id_pushover_user_token" class="col-12 col-form-label">User Key</label>
    ...
    <div v-else>
      <p class="text-warning">Please configure the following variables in the "docker-compose.override.yml" file to enable Pushover notifications:</p>
      <ul class="text-warning">
        <li>PUSHOVER_APP_TOKEN</li>
      </ul>
    </div>
  </section>
</template>
  ...
  created() {
    if (!this.notificationChannel.channelInfo) {
      ...
      if (this.notificationChannel.channelInfo.config) {
        this.userKey = this.notificationChannel.channelInfo.config.user_key
      }
    }
  },

  watch: {
    userKey(newValue, oldValue) {
      if (oldValue === null) {
        return
      }
