                ...
                <h3 v-if="trialDaysLeft >= 29" class="py-3">
                  <i18next :translation="$t('Your 30-Day {localizedDom} Free Trial Has Started!')">
                    <template #localizedDom>
                      <a class="link" target="_blank" :href="this.getDocLink('/user-guides/upgrade-to-pro/')">{{$t("Pro Plan")}}</a>
                    </template>
                  </i18next>
                </h3>
        ...
        <div v-else>
          ...
          <div v-else>
            <div>
              ...
              <p class="text-muted">
                <a :href="this.getDocLink('/user-guides/octoprint-tunneling/')" target="_blank">{{platformDisplayName}} {{$t("Tunnel")}}</a>
              ...
              <b-alert v-if="!user.is_pro" variant="warning" dismissible class="my-3" show>
                <div>
                  ...
                  <i18next :translation="$t('Tunnel usage of a free account is {localizedDom} to enjoy unlimited tunnel usage.')">
                    <template #localizedDom>
                      <a :href="this.getDocLink('/user-guides/octoprint-tunneling/#why-is-the-limit-on-free-account-only-100mb')" target="_blank">{{$t("capped at 300MB per month")}}</a>. {{$t("You can")}} <a href="http://app.obico.io/ent_pub/pricing/" target="_blank">{{$t("upgrade to the {brandName} app Pro plan for 1 Starbucks a month",{brandName:$syndicateText.brandName})}}</a>
                    </template>
                  </i18next>
                </div>
              </b-alert>
            ...
            <div class="account-details">
              <p>
                {{ $t("With the Free {platformDisplayName} Tunnel by {brandName}, you can now use {appDisplayName} to",{platformDisplayName,brandName:$syndicateText.brandName,appDisplayName}) }}
                  <a :href="this.getDocLink('/user-guides/octoprint-tunneling/#why-is-the-limit-on-free-account-only-100mb')" target="_blank"> {{$t("securely control and monitor your printer from anywhere")}}</a>
              </p>
              <ul class="text-muted">
