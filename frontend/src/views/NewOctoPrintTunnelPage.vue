                ...
                <h3 v-if="trialDaysLeft >= 29" class="py-3">
                  <i18next :translation="$t('Your 30-Day {localizedDom} Free Trial Has Started!')">
                    <template #localizedDom>
                      <a class="link" target="_blank" :href="getDocLink('/user-guides/upgrade-to-pro/')">{{$t("Pro Plan")}}</a>
                    </template>
                  </i18next>
                </h3>
        ...
        <div v-else>
          ...
          <div v-else>
            <div>
              ...
              <p class="text-muted">
                <a :href="getDocLink('/user-guides/octoprint-tunneling/')" target="_blank">{{platformDisplayName}} {{$t("Tunnel")}}</a>
            ...
            <div class="account-details">
              <p>
                {{ $t("With the Free {platformDisplayName} Tunnel by {brandName}, you can now use {appDisplayName} to",{platformDisplayName,brandName:$syndicateText.brandName,appDisplayName}) }}
                  <a :href="getDocLink('/user-guides/octoprint-tunneling/#why-is-the-limit-on-free-account-only-100mb')" target="_blank"> {{$t("securely control and monitor your printer from anywhere")}}</a>
              </p>
              <ul class="text-muted">
