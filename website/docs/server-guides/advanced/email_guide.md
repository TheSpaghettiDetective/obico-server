...
Ensure you are correctly re-creating your Docker containers with the command ```docker compose up -d``` (use sudo in front for Linux) when making changes during troubleshooting, otherwise your changes may not take effect! Put your customizations in a `docker-compose.override.yml` file rather than editing `docker-compose.yml` directly — the latter is updated by the project and editing it will cause conflicts on `git pull`.
